...
RSpec.describe Payouts::Create, type: :service do
  describe '#execute' do
    subject(:execute) { Payouts::Create.new(Date.parse('2023-06-01')).call }
    ...
    context 'when there are some suitable orders for payout' do
      ...
      it { expect { execute }.to change(Payout, :count).from(0).to(2) }

      it 'creates payout for daily merchant for the corresponding day' do
        execute

        payouts = Payout.where(merchant_id: daily_merchant.id)
      ...
      it 'creates payout for weekly merchant for the corresponding week' do
        ...
        payouts = Payout.where(merchant_id: weekly_merchant.id)
        aggregate_failures do
          expect(payouts.size).to eq(1)
          expect(payouts.first.payout_date.to_s).to eq('2023-06-01')
      ...
      it 'updates related orders by created payout ids' do
        ...
        aggregate_failures do
          expect(Order.where(payout_id: nil).count).to eq(2)
